...
function Asteroid({ asteroid, isSelected, onSelect }) {
  ...
  const { effectiveTime } = useTime();

  console.log('Rendering asteroid:', asteroid.name, 'with orbit:', asteroid.orbit);

  // Calculate position based on real orbital data or fallback to deterministic
  useMemo(() => {
    if (asteroid.position) {
      // Use real position from API
      position.current.set(
        asteroid.position.x / 1000000, // Convert to appropriate scale
        asteroid.position.y / 1000000,
        asteroid.position.z / 1000000
      );
      console.log(`Asteroid ${asteroid.name} real position:`, asteroid.position);
    } else if (asteroid.orbit) {
      // Fallback to deterministic positioning
      ...
      const a = 8 + seededRandom(seed + 1) * 4;
      ...
      console.log(`Asteroid ${asteroid.name} fallback position:`, { x, y, z });
    } else {
      // No orbital data, place at fixed position
      const seed = asteroid.id ? asteroid.id.split('').reduce((a, b) => a + b.charCodeAt(0), 0) : 1000;
      const seededRandom = (seed) => (Math.sin(seed) * 10000) % 1;
  ...
  }, [asteroid.id, asteroid.orbit, asteroid.orbitProgress, asteroid.position, effectiveTime]);
